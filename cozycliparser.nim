...
## +++++++++++++++++++++++++
##
## Because `HelpText` is built at compile time, injecting dynamic runtime
## values (like the current directory or an environmantal variable) into help
## descriptions is done via a lazy interpolation (string replacement) hook.
##
## Register a global interpolator using `helpInterpolator`_ or the
## `setHelpInterpolator`_ template. The registered closure is invoked exactly
## when the help text is converted to a string (`$`) or displayed.
##
...
Options:
  ...
  -h, --help      Show this help and exit"""
##
## Now, whenever `Cli.help.display()` is called - whether manually, via the
## auto-injected `-h` flag, or within the autogenerated `onError` handler - the
## placeholders are cleanly replaced.
##
## .. important:: Registered interpolator fires multiple times, for each plain
##    span of the help text, so calls used inside will be repeatedly evaluated.
##    Evaluate them preemptively and access or cache via captured variables to
##    eschew the computational cost. In the example above, the whole replacement
##    array passed to `multiReplace` could have been neatly stored outside of
##    the closure.
##
...
template resolve(span: HelpSpan): string =
  if bpCtx.interpolator != nil and span.tag == htPlain:
    bpCtx.interpolator(span.text)
  else:
    span.text

proc write*(f: File; doc: HelpText) =
  ## Writes `doc` unstyled.
  for span in doc: f.write(span.resolve())
proc `$`*(doc: HelpText): string =
  for span in doc: result.add span.resolve()
...
proc helpInterpolator*(handler: proc (s: string): string) =
  ...
  bpCtx.interpolator = handler

template setHelpInterpolator*(body: untyped) =
  ## Convenience wrapper for `helpInterpolator`_.
  ## Injects `s` for the closure's string parameter.
  helpInterpolator(proc(s {.inject.}: string): string = body)
